...
{
	// ⚡ Bolt Optimizasyonu: std::to_string çağrıları ve concatenation yerine snprintf ile tek bir char array üzerinden string üreterek performans artışı sağlandı.
	// Ayrıca saat/dakika tek hane düştüğünde sıfır dolgusu yapılması da sağlandı.
	char buf[16];
	std::snprintf(buf, sizeof(buf), "%d:%02d", int(td / 60) % 12, int(td % 60));
...
void zaman::sat_turk_v_d()
{
	...
	char buf[32];
	// ⚡ Bolt Optimizasyonu: std::string birleştirme (append, +) yerine snprintf kullanarak string kopyalama sayısını ve bellek ayırmalarını azalttık. Aynı zamanda dakika ve saniyenin tek haneli kalmasını engelledik.
	std::snprintf(buf, sizeof(buf), "%d:%02d:%02d", zaman::saat, zaman::dakika, zaman::saniye);
