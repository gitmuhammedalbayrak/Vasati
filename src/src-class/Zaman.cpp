...
void zaman::vkt_h_v_d()
{
	...
	static const pugi::xml_node* cached_xml_nodes = []() {
	...
	if (current_day >= 0 && current_day < 400 && cached_xml_nodes[current_day]) {
		zaman::xml_bu_gun = std::string(cached_xml_nodes[current_day].text().get());
	} else {
		zaman::xml_bu_gun = "";
	}
	...
	if (next_day >= 0 && next_day < 400 && cached_xml_nodes[next_day]) {
		zaman::xml_bu_gun = std::string(cached_xml_nodes[next_day].text().get());
	} else {
		zaman::xml_bu_gun = "";
	}
...
void zaman::sat_turk_v_d()
{
	...
	char time_buf[32];
	std::snprintf(time_buf, sizeof(time_buf), "%d:%d:%d", zaman::saat, zaman::dakika, zaman::saniye);
	zaman::simdiki_zaman_turk = time_buf;
