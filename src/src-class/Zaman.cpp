...
void zaman::sat_turk_v_d()
{
	...
	// Bolt Optimizasyonu: std::to_string yerine dogrudan ASCII dizisi kullanilarak
	// dinamik bellek ayrilimi onlenmis ve performans artirilmistir.
	char buf[12];
	int len = 0;
	if (zaman::saat >= 10) { buf[len++] = '0' + (zaman::saat / 10); buf[len++] = '0' + (zaman::saat % 10); } else { buf[len++] = '0' + zaman::saat; }
	buf[len++] = ':';
	if (zaman::dakika >= 10) { buf[len++] = '0' + (zaman::dakika / 10); buf[len++] = '0' + (zaman::dakika % 10); } else { buf[len++] = '0' + zaman::dakika; }
	buf[len++] = ':';
	if (zaman::saniye >= 10) { buf[len++] = '0' + (zaman::saniye / 10); buf[len++] = '0' + (zaman::saniye % 10); } else { buf[len++] = '0' + zaman::saniye; }

	zaman::simdiki_zaman_turk.append(buf, len);
