...
{
	char buffer[16]; std::snprintf(buffer, sizeof(buffer), "%d:%02d", int(td / 60) % 12, int(td % 60)); return std::string(buffer);
}
...
void zaman::sat_turk_v_d()
{
	...
	char buffer_time[32]; std::snprintf(buffer_time, sizeof(buffer_time), "%02d:%02d:%02d", zaman::saat, zaman::dakika, zaman::saniye); zaman::simdiki_zaman_turk = buffer_time;
