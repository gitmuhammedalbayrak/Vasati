...
std::string zaman::td_to_vakt(unsigned int td)
{
	...
	int idx = 0;
	if (h >= 10) { buf[idx++] = '0' + (h / 10); buf[idx++] = '0' + (h % 10); }
	else { buf[idx++] = '0' + h; }
	buf[idx++] = ':';
	if (m >= 10) { buf[idx++] = '0' + (m / 10); buf[idx++] = '0' + (m % 10); }
	else { buf[idx++] = '0' + m; }
	buf[idx] = '\0';
...
void zaman::vkt_h_v_d()
{
	...
	static const char* cached_nodes_arr[400] = {nullptr};
	static bool cached_nodes_init = []() {
		...
		for (pugi::xml_node pt = node.child("prayertimes"); pt; pt = pt.next_sibling("prayertimes")) {
			...
			if (day >= 0 && day < 400) {
				cached_nodes_arr[day] = pt.text().get();
			}
		}
		return true;
	}();

	if (zaman::h_rakam_gun_senenin >= 0 && zaman::h_rakam_gun_senenin < 400 && cached_nodes_arr[zaman::h_rakam_gun_senenin]) {
		zaman::xml_bu_gun = cached_nodes_arr[zaman::h_rakam_gun_senenin];
	} else {
		zaman::xml_bu_gun = "";
	}
	...
	if (next_day >= 0 && next_day < 400 && cached_nodes_arr[next_day]) {
		zaman::xml_bu_gun = cached_nodes_arr[next_day];
	} else {
		zaman::xml_bu_gun = "";
	}
...
void zaman::sat_turk_v_d()
{
	...
	char buf[9];
	unsigned int s = zaman::saat;
	unsigned int d = zaman::dakika;
	unsigned int sn = zaman::saniye;
	int idx = 0;
	if (s >= 10) { buf[idx++] = '0' + (s / 10); buf[idx++] = '0' + (s % 10); }
	else { buf[idx++] = '0' + s; }
	buf[idx++] = ':';
	if (d >= 10) { buf[idx++] = '0' + (d / 10); buf[idx++] = '0' + (d % 10); }
	else { buf[idx++] = '0' + d; }
	buf[idx++] = ':';
	if (sn >= 10) { buf[idx++] = '0' + (sn / 10); buf[idx++] = '0' + (sn % 10); }
	else { buf[idx++] = '0' + sn; }
	buf[idx] = '\0';
