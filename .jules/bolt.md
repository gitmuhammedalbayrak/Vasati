...
## 2024-05-18 - [Bellek Şişmesi (Memory Bloat) ve Performans Düşüşü: `std::string::append` vs `=`]
**Learning:** `zaman` sınıfı metodlarında (ör. `vkt_turk_v_d`, `sat_turk_v_d`) string birleştirme için `std::string::append` kullanımı, bu metodlar aynı obje üzerinde tekrar tekrar çağrıldığında önceki değerlerin üzerine ekleme yaparak kontrolsüz bellek büyümesine (memory bloat) sebep oldu (10.000 döngüde string boyutu 80008 karaktere çıktı). Atama (`=`) kullanıldığında ise gereksiz heap tahsislerinden kaçınılıyor ve süre 34 ms'den 25 ms'ye düşüyor.
**Action:** Sınıf metodlarında tekrar tekrar hesaplanan/güncellenen `std::string` alanlarında `.append()` yerine her zaman doğrudan atama (`=`) kullanılarak önceden var olan buffer ezilmelidir.
