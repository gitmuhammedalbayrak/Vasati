...
## 2026-05-27 - [Fix string append memory leak and missing zero-padding]
**Learning:** Repeatedly calling `.append()` on a string class member variable (`zaman::simdiki_zaman_turk` and prayer times) inside functions that are called repeatedly (like in a loop or per update) causes unbounded string growth and massive memory reallocation overhead (O(N^2)). Additionally, manually converting integers to strings with `std::to_string` and concatenating them (e.g. `10 + ":" + 5`) loses necessary zero-padding, causing display bugs like `10:5` instead of `10:05`.
**Action:** Replace `.append()` calls with direct assignment (`=`). When formatting time strings, use `std::snprintf` with a fixed-size local `char` buffer (e.g. `"%d:%02d:%02d"`) and assign the result. This fixes unbounded string growth, eliminates allocations during concatenation, and ensures proper zero-padding display.
