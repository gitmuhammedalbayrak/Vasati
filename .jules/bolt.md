...
## 2023-10-27 - [td_to_vakt ve simdiki_zaman_turk string atama optimizasyonu]
**Learning:** `std::string::append` metodu mevcut stringin sonuna ekleme yapar. Constructor'da (veya döngü içinde) sürekli `.append` kullanıldığında, her çağrıda string büyür ve hafıza (memory) sızıntısına/gereksiz tahsislere neden olur. Aynı zamanda yanlış mantıksal değer üretir. `std::string` atamalarında `=` operatörünü kullanmak ve `std::to_string` zincirlemesi yerine (örn: `std::to_string(saat) + ":" + std::to_string(dakika)`) `std::snprintf` ile sabit boyutlu stack buffer (char array) kullanmak, hem gereksiz hafıza tahsislerini (allocation) engeller hem de performansı artırır.
**Action:** Döngüsel olarak veya çok sık çağrılan fonksiyonlarda (örneğin class constructor'larında) `std::to_string` ve `+` (veya `.append`) yerine `std::snprintf` ve stack tabanlı `char` buffer'ları tercih edilmelidir. String değerlerini yenilerken `.append()` yerine her zaman doğrudan atama (`=`) kullanılmalıdır.
