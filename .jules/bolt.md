...
## 2024-05-18 - [Fix infinite string append in `zaman::sat_turk_v_d`]
**Learning:** `simdiki_zaman_turk` ve diğer zaman stringleri, tekrarlanan `zaman::vkt_turk_v_d` ve `zaman::sat_turk_v_d` çağrılarında `.append()` kullanılarak inşa ediliyordu. Bu durum, nesne birden fazla kez güncellendiğinde veya bu fonksiyonlar tekrar çağrıldığında (örn. ana döngüde saat güncellenirken) stringin sonsuza kadar büyümesine, bellek sızıntısına benzer bir duruma ve hatalı saat gösterimine yol açıyordu.
**Action:** `zaman::vkt_turk_v_d` ve `zaman::sat_turk_v_d` içinde `.append()` kullanımını doğrudan atama (`=`) ile değiştirdik. Formatlama için `std::snprintf` kullanarak sıfır dolgulu (zero-padded) ve güvenli string oluşturduk. Ayrıca, diğer zaman verilerini oluştururken `std::string::append` kullanımı yerine doğrudan `=` ataması kullanıldı. Performans testleri, 10000 tekrarlı döngüde oluşan hatalı `80008` karakterlik string uzunluğunun beklenen `8` karakter uzunluğuna düştüğünü gösterdi.
