...
## 2026-07-24 - [String .append() ile Sürekli Bellek Büyümesi]
**Learning:** Sınıf içinde önceden atanmış `std::string` üye değişkenlerini birden fazla kez güncellerken (örneğin döngüde veya objeyi birden çok kez kullanırken) `+` ya da `.append()` kullanmak hem string boyutunun sınırsızca büyümesine neden oluyor (gizli bellek sızıntısı), hem de sürekli allocation overhead'i yarattığı için performansı olumsuz etkiliyor. Ayrıca basit saat/dakika formatlamalarında `std::to_string` kullanmak sıfır dolgusu (zero-padding) gibi formatlama sorunlarına yol açıyor.
**Action:** Nesne içi string güncellemelerinde doğrudan atama (`=`) kullanılmalı ve sayıları string formatına dönüştürürken stack üzerinde char dizisi tahsis edilip `std::snprintf` fonksiyonu ile formatlanmalıdır. Bu sayede bellek ayırma minimuma iner, sınırsız string büyümesi engellenir ve format kontrolü kolaylaşır.
