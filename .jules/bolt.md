...
## 2024-05-27 - [td_to_vakt string allocation optimisation]
**Learning:** In heavily called string formatting functions like `td_to_vakt`, multiple `std::to_string` allocations and string concatenations (`+` or `.append()`) cause severe memory allocation bottlenecks.
**Action:** Use a manual, fixed-size stack character buffer (e.g., `char buf[]`) and direct ASCII arithmetic (`'0' + value`) to entirely bypass temporary object creation and formatting logic, yielding significant performance improvements (~4x faster execution in benchmarks).
