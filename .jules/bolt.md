...
## 2026-05-31 - [String Formatting Bottleneck Optimization]
**Learning:** In heavily called functions like `td_to_vakt` and `sat_turk_v_d()`, multiple `std::to_string` allocations and string concatenations (`+` or `.append()`) cause severe memory allocation bottlenecks. Using a manual, fixed-size stack character buffer (e.g., `char buf[]`) and direct ASCII arithmetic (`'0' + value`) entirely bypasses temporary object creation and formatting logic, yielding significant performance improvements.
**Action:** I will replace `std::to_string` and `+` based string concatenation with a fast stack-allocated buffer and simple arithmetic in `td_to_vakt` and `sat_turk_v_d`.
