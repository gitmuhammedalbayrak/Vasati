...
## 2026-07-04 - [`td_to_vakt` ve string formatlama performans optimizasyonu]
**Learning:** `std::to_string` fonksiyonu, string birleştirme operatörleri (`+` ve `.append()`) ile birlikte kullanıldığında, sürekli ve dinamik bellek tahsisi (heap allocation) nedeniyle ciddi performans kayıplarına yol açar. Zaman hesaplama gibi sık çağrılan döngülerde bu gecikme daha da belirginleşir. Ek olarak, `.append()` metodunun ardışık çağrılarda kullanılması string'in kontrolsüz büyümesine sebep olur.
**Action:** Sabit uzunlukta ve öngörülebilir kısa metinler (zaman formatı vs.) oluştururken, `std::to_string` yerine stack üzerinde yer ayrılmış `char` buffer'ları ve `std::snprintf` kullandık. Ayrıca, kontrolsüz büyümeyi engellemek için doğrudan atama (`=`) yöntemine geçtik. Bu sayede bellek tahsisi yükü engellenerek güvenilir bir hız artışı sağlandı.
